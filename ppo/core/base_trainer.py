...
class BaseTrainer:
    ...
    def compute_action(self, obs, deterministic=False):
        ...
        # it seems that deterministic is not used here
...
class ActorCritic(nn.Module):
    def __init__(self, input_shape, num_actions, discrete):
        ...
        if input_shape[1:] == (42, 42):
            ...
            self.conv1 = init_(nn.Conv2d(12, 16, kernel_size=4, stride=2))  # (-1, 16, 47, 47)
        ...
        elif input_shape[1:] == (100, 50):  # For cCarRacing-v0
            self.conv1 = init_(nn.Conv2d(12, 16, kernel_size=4, stride=2))  # (-1, 16, 47, 47)
            self.conv2 = init_(nn.Conv2d(16, 32, kernel_size=4, stride=2))  # (-1, 32, 22, 22)
            self.conv3 = init_(nn.Conv2d(32, 64, kernel_size=4, stride=2))  # (-1, 64, 10, 10)
            self.conv4 = init_(nn.Conv2d(64, 128, kernel_size=4, stride=2))  # (-1, 128, 4, 4)
            self.conv5 = init_(nn.Conv2d(128, 256, kernel_size=1, stride=1))  # (-1, 256, 1, 1)
            self.hidden = nn.Linear(256*4, 100)
        else:
