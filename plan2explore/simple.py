...
from envsimple import CONTROL_SUITE_ENVS, Env, GYM_ENVS, EnvBatcher
...
parser.add_argument('--env', type=str, default='Tetris-v0', choices=GYM_ENVS, help='Gym/Control Suite environment')
parser.add_argument('--symbolic-env', action='store_true', help='Symbolic features')
parser.add_argument('--max-episode-length', type=int, default=10000, metavar='T', help='Max episode length')
parser.add_argument('--experience-size', type=int, default=10000000, metavar='D', help='Experience replay size')  # Original implementation has an unlimited buffer size, but 1 million is the max experience collected anyway
...
parser.add_argument('--embedding-size', type=int, default=128, metavar='E', help='Observation embedding size')  # Note that the default encoder for visual observations outputs a 1024D vector; for other embedding sizes an additional fully-connected layer is used
...
parser.add_argument('--add_reward', action='store_true', help='Checkpoint experience replay')
...
env = Env(args.env, args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth, args)
test_envs = EnvBatcher(Env, (args.env, args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth, args), {}, args.test_episodes)
...
if args.experience_replay is not '' and os.path.exists(args.experience_replay):
    ...
    D2 = ExperienceReplay(args.experience_size, args.symbolic_env, env.observation_size, env.action_size, args.bit_depth, args.device)
    ...
    if args.env=="Tetris-v0":
        ...
        if args.experience_buffer:
            ...
            for i in range(total):
                ...
                D2.append(obs,action,reward,done)
...
def clone():
    if True:
        if args.experience_buffer and args.clone:
            ...
            observations, actions, rewards, nonterminals = D2.sample(args.batch_size, args.chunk_size) # Transitions start at time t = 0
    ...
    for s in tqdm(range(args.collect_interval)):
        ...
        if args.algo=="p2e":
            ...
            for mdl in range(len(onestep_models)):
                ...
                loss = ((prediction - target_prediction.detach()).pow(2)).mean(dim=(0,1))
