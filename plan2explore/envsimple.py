...
GYM_ENVS = ["Tetris-v0","TetrisA-v0","TetrisA-v1",'Pendulum-v0', 'MountainCarContinuous-v0', 'Ant-v2', 'HalfCheetah-v2', 'Hopper-v2', 'Humanoid-v2', 'HumanoidStandup-v2', 'InvertedDoublePendulum-v2', 'InvertedPendulum-v2', 'Reacher-v2', 'Swimmer-v2', 'Walker2d-v2']
...
class GameEnv():
	...
	def _get_board_ram(self):
		board=np.array(self._env.game_state.board)
		board[board!="."]=1
		board[board=="."]=0
		board=board.astype(np.float32).transpose(1,0)
		return board
	...
	def backup(self):
		self.checkpoint=copy.deepcopy(self._env.game_state)
	
	def restore(self):
		self._env.game_state=copy.deepcopy(self.checkpoint)
