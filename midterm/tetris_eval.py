...
class Tetris:
    ...
    def reset(self):
        ...
        self.gameover = False
        return self.get_state_properties(self.board)[0]
    ...
    def get_state_properties(self, board):
        ...
        if self.simplified_feature:
            ...
            return torch.FloatTensor(np.concatenate((heights, holes, 
                                [num_holes, bumpiness, height, lines_cleared, self.ind]))), old_board
            # heights = self.get_height_vector(board)
            # holes = self.get_hole_vector(board, heights)
            # return torch.FloatTensor(np.concatenate((heights, holes, [lines_cleared, self.ind]))), old_board
    ...
    def get_next_states(self):
        ...
        for i in range(num_rotations):
            ...
            for x in range(valid_xs + 1):
                ...
                states[(x, i)],old_board = self.get_state_properties(board)
                board_lst[(x, i)]=(np.array(old_board),pos["y"],states[(x, i)][-2].item(),np.array(board))
            curr_piece = self.rotate(curr_piece)
        return states, board_lst
